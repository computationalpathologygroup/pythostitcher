...
class Fragment:
    ...
    def __init__(self, fragment_name, kwargs):
        ...
        self.im_path = f"{self.data_dir}/images/{self.fragment_name}.tif"
        self.mask_path = f"{self.data_dir}/masks/{self.fragment_name}.tif"
    ...
    def read_transforms(self):
        """
        Method to read the transformation info for each fragment. This is a .txt file
        ...
        if not os.path.isfile(f"{self.data_dir}/rotations.txt"):
            raise ValueError("Couldn't find file with fragment transformations")

        # Get line with fragment info
        with open(f"{self.data_dir}/rotations.txt") as f:
            lines = f.read().splitlines()

        q_idx_line = np.argmax([self.fragment_name.lower() in i.lower() for i in lines])
    ...
    def read_image(self):
        ...
        if os.path.isfile(self.im_path):
            ...
            if self.initial_angle_k != 0:
                self.original_image = np.rot90(
                    self.original_image, k=self.initial_angle_k
                )
            if self.hflip:
    ...
    def preprocess_colour_image(self):
        """
        Method to preprocess original image to colour image. This is not actually
        required for the stitching algorithm but may be used for plotting of the result.
        """
    ...
    def apply_masks(self):
        ...
        if self.iteration == 0:
            ...
            prev_res_image_path = (
                ...
                f"fragment_{self.fragment_name}_gray.png"
            )
    ...
    def save_fragment(self):
        ...
        self.fragment_savepath = (
            f"{self.results_dir}/images/{self.slice_idx}/"
            f"{self.res_name}/fragment_{self.fragment_name}"
        )
    ...
    def load_images(self):
        ...
        # rather than RGB.
        basepath_load = f"{self.results_dir}/images/{self.slice_idx}/{self.res_name}"
        self.mask = cv2.imread(
            f"{basepath_load}/fragment_{self.fragment_name}_mask.png"
        )
        if len(self.mask.shape) == 3:
            self.mask = cv2.cvtColor(self.mask, cv2.COLOR_BGR2GRAY)

        self.gray_image = cv2.imread(
            f"{basepath_load}/fragment_{self.fragment_name}_gray.png"
        )
        self.gray_image = cv2.cvtColor(self.gray_image, cv2.COLOR_BGR2GRAY)
        self.colour_image = cv2.imread(
            f"{basepath_load}/fragment_{self.fragment_name}_colour.png"
        )
    ...
    def get_bbox_corners(self, image):
        ...
        if self.num_fragments == 2:
            ...
            # coordinates.
            if self.fragment_name == "top":
                corner_idxs = np.argsort(self.bbox_corners[:, 1])[-2:]
            elif self.fragment_name == "bottom":
                corner_idxs = np.argsort(self.bbox_corners[:, 1])[:2]
            elif self.fragment_name == "left":
                corner_idxs = np.argsort(self.bbox_corners[:, 0])[-2:]
            elif self.fragment_name == "right":
                corner_idxs = np.argsort(self.bbox_corners[:, 0])[:2]
        ...
        # Case of 4 fragments
            ...
            if self.fragment_name == "ul":
                corner_a_idx = np.argmax(
                    (mask_corners[:, 0] > center[0]) * (mask_corners[:, 1] > center[1])
                )
            elif self.fragment_name == "ur":
                corner_a_idx = np.argmax(
                    (mask_corners[:, 0] < center[0]) * (mask_corners[:, 1] > center[1])
                )
            elif self.fragment_name == "ll":
                corner_a_idx = np.argmax(
                    (mask_corners[:, 0] > center[0]) * (mask_corners[:, 1] < center[1])
                )
            elif self.fragment_name == "lr":
                corner_a_idx = np.argmax(
                    (mask_corners[:, 0] < center[0]) * (mask_corners[:, 1] < center[1])
                )
    ...
    def get_initial_transform(self):
        ...
        pad, pad_x, pad_y = pad_dict[self.fragment_name]
    ...
    def get_tformed_images_pair(self, fragments):
        ...
        # Determine which complementary fragment is required
        with open("../config/config.json") as f:
            file = json.load(f)
            complementary_fragments = file["complementary_fragments_pair"]

        # Get the complementary fragment
        other_fragment_name = complementary_fragments[self.fragment_name]
        other_fragment = next(
            (i for i in fragments if i.fragment_name == other_fragment_name), None
        )
        ...
        if self.fragment_name in ["ul", "left"]:
        ...
        elif self.fragment_name in ["ur", "right"]:
        ...
        if self.fragment_name in ["left", "right", "top", "bottom"]:
    ...
    def get_tformed_images_total(self, fragments):
        ...
        with open("../config/config.json") as f:
            file = json.load(f)
            complementary_fragments = file["complementary_fragments_total"]
        ...
        if self.fragment_name in ["ul", "ur"]:
        ...
        elif self.fragment_name in ["ll", "lr"]:
    ...
    def get_edges(self):
        ...
        h_edge, v_edge = edge_dict[self.fragment_name]

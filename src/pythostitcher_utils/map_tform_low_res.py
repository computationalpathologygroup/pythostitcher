...
def map_tform_low_res(parameters):
    """
    Custom function to upsample the previously acquired tform matrix. This upsampling
    can be performed linearly since the different images from different resolutions
    were preprocessed to be the same ratio relative to the change in resolution.
    ...
    ratio = (
        parameters["resolutions"][parameters["iteration"]]
    ...
    prev_filepath_final_tform = (
        f"{parameters['sol_save_dir']}/" f"tform/{prev_resname}_tform_final.npy"
    )

    current_resname = get_resname(parameters["resolutions"][parameters["iteration"]])
    current_filepath_initial_tform = (
        f"{parameters['sol_save_dir']}/" f"tform/{current_resname}_tform_initial.npy"
    )
    ...
    # Although we technically don't need to save this initial tform for this resolution, this
    # can come in handy for comparing the results of the genetic algorithm for a given
    # resolution.
